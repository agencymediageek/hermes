...
import { Check, CircleStop, ExternalLink, FileText, Pencil, RefreshCw, Send, ShieldCheck, X } from 'lucide-react';
...
export default function ChatView({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => {
    if (!requestedSessionId || streaming || selectedRef.current === requestedSessionId) return;
    let cancelled = false;
    const local = sessionsRef.current.find((candidate) => candidate.id === requestedSessionId);
    if (local) {
      selectSession(local);
      return () => { cancelled = true; };
    }
    turbohermes.chatSession(requestedSessionId).then((fetched) => {
    ...
    return () => { cancelled = true; };
  }, [requestedSessionId, selectSession, streaming]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!selectedRef.current && items.length) selectSession(items.find((item) => item.id === requestedSessionId) || items[0]);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Unable to load chat sessions');
    } finally { setLoading(false); }
  }, [requestedSessionId, selectSession]);
  useEffect(() => { load(); }, [load]);
  useEffect(() => {
    const handleSessionUpdated = (event: Event) => {
      const updated = (event as CustomEvent<{ session?: ChatSession }>).detail?.session;
      if (!updated) return;
      sessionsRef.current = sessionsRef.current.map((item) => item.id === updated.id ? updated : item);
      if (selectedRef.current === updated.id) setSelected(updated);
    };
  ...
  const openRename = () => {
    if (!selected) return;
    setRenameValue(selected.projectTitle?.trim() || '');
    setRenameError(null);
    setRenameOpen(true);
  };

  const renameSelected = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const projectTitle = renameValue.trim();
    if (!projectTitle) {
      setRenameError('Enter a project or idea name to continue.');
      return;
    }
    ...
    if (!selected || renamingRef.current) return;
    renamingRef.current = true;
    setRenaming(true);
    setRenameError(null);
    try {
      const updated = await turbohermes.renameChatSession(selected.id, projectTitle);
      sessionsRef.current = sessionsRef.current.map((item) => item.id === updated.id ? updated : item);
      setSelected(updated);
      window.dispatchEvent(new CustomEvent('hermes:chat-session-updated', { detail: { session: updated } }));
      setRenameOpen(false);
    } catch (cause) {
      setRenameError(cause instanceof Error ? cause.message : 'Unable to rename this chat.');
    } finally {
